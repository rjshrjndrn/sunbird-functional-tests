...
public class ReadPageSettingsTest extends BaseCitrusTestRunner {
	...
	public Object[][] readPageSettingsSuccessDataProvider() {
		...
		return new Object[][] {
			new Object[] {
					TEST_NAME_READ_PAGE_SETTINGS_SUCCESS_WITH_VALID_PAGE_ID, true, HttpStatus.OK,
			}
		};
	}
	...
	public void testReadPageSettingsSuccess(
		...
		if (testName.equalsIgnoreCase(TEST_NAME_READ_PAGE_SETTINGS_SUCCESS_WITH_VALID_PAGE_ID)) {
			variable("pageName", PAGE_NAME);
			beforeTestPageSettings();
		}

		String url = getReadPageUrl() + "/pageId";

		performGetTest(
				this, TEMPLATE_DIR, testName, url, isAuthRequired, httpStatusCode, RESPONSE_JSON);
	}
	private void beforeTestPageSettings() {
		PageUtil.createPage(
				this,
				testContext,
